...
class Trie {
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    std::unique_ptr<TrieNode> *parent_node = &root_;

    for (size_t i = 0; i < key.length(); i++) {
      char ch = key[i];
      if (i < key.length() - 1) {
        if (!parent_node->get()->HasChild(ch)) {
          parent_node->get()->InsertChildNode(ch, std::make_unique<TrieNode>(ch));
        }
        parent_node = parent_node->get()->GetChildNode(ch);
      } else {
        auto terminal_node = parent_node->get()->GetChildNode(ch);
        // case 1
        if (terminal_node == nullptr) {
          parent_node->get()->InsertChildNode(ch, std::make_unique<TrieNodeWithValue<T>>(ch, value));
          latch_.WUnlock();
          return true;
        }
        // case 2
        if (!terminal_node->get()->IsEndNode()) {
          auto new_node = new TrieNodeWithValue(std::move(**terminal_node), value);
          terminal_node->reset(new_node);
          latch_.WUnlock();
          return true;
        }
        // case 3
  ...
  bool Remove(const std::string &key) {
    ...
    std::stack<std::tuple<char, std::unique_ptr<TrieNode> *>> node_stack;
    for (size_t i = 0; i < key.length(); i++) {
      auto ch = key[i];
      ...
      if (i == key.length() - 1) {
        child_node->get()->SetEndNode(false);
      }
      node_stack.emplace(ch, parent_node);
      parent_node = child_node;
    }

    while (!node_stack.empty()) {
      auto node_tuple = node_stack.top();
      auto ch = std::get<0>(node_tuple);
      parent_node = std::get<1>(node_tuple);
      node_stack.pop();
      auto child_node = parent_node->get()->GetChildNode(ch);
      if (!child_node->get()->IsEndNode() && !child_node->get()->HasChildren()) {
